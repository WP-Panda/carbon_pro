...
/*
Plugin Name:  WPP Регенерация картинок
...
class ForceRegenerateThumbnails {
	...
	function __construct() {
		...
		$this->wpp_sizes = [
			BR_THUMB_SIZE,   // миниатюра для везде
			[ 'width' => 1920, 'height' => 1280 ], // полноэкранный слайдер
			[ 'height' => 780 ], // комп карусель
			[ 'height' => 680 ], // планшет карусель
			[ 'height' => 400 ], // мобила карусель
            [],
		];
	}
	...
	function add_admin_menu() {
		$this->menu_id = add_management_page( __( 'Force Regenerate Thumbnails', 'force-regenerate-thumbnails' ), __( 'Force Regenerate Thumbnails', 'force-regenerate-thumbnails' ), $this->capability, 'force-regenerate-thumbnails', array(
			&$this,
	...
	function add_bulk_actions( $actions ) {
		...
		$actions['bulk_force_regenerate_thumbnails'] = __( 'Регенерация картинок', 'force-regenerate-thumbnails' );
	...
	function add_bulk_actions_via_javascript() {
        ...
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                $('select[name^="action"] option:last-child').before('<option value="bulk_force_regenerate_thumbnails"><?php echo esc_attr( __( 'Регенерация картинок', 'force-regenerate-thumbnails' ) ); ?></option>');
            });
        </script>
		<?php
	}
	...
	function force_regenerate_interface() {
        ...
        <div class="wrap regenthumbs">
            <h2><?php _e( 'Регенерация картинок', 'force-regenerate-thumbnails' ); ?></h2>
			...
			if ( ! empty( $_POST['force-regenerate-thumbnails'] ) || ! empty( $_REQUEST['ids'] ) ) {
                ...
                <noscript><p>
                        <em><?php _e( 'You must enable Javascript in order to proceed!', 'force-regenerate-thumbnails' ) ?></em>
                    </p></noscript>
                ...
                <p><input type="button" class="button hide-if-no-js" name="regenthumbs-stop" id="regenthumbs-stop"
                          value="<?php _e( 'Abort Process', 'force-regenerate-thumbnails' ) ?>"/></p>
                ...
                <p>
					...
					<?php printf( __( 'Success: %s', 'force-regenerate-thumbnails' ), '<span id="regenthumbs-debug-successcount">0</span>' ); ?>
                    <br/>
			...
			?>
                <form method="post" action="">
                    ...
                    <input type="submit" class="button-primary hide-if-no-js" name="force-regenerate-thumbnails"
                           id="force-regenerate-thumbnails"
                           value="<?php _e( 'Regenerate All Thumbnails', 'force-regenerate-thumbnails' ) ?>"/>
                    </p>
	...
	function ajax_process_image() {
		...
		try {
			...
			if ( is_null( $image ) ) {
				throw new Exception( sprintf( __( 'Ошибка: %d не действительный ID.', 'force-regenerate-thumbnails' ), $id ) );
			}
			...
			if ( ! current_user_can( $this->capability ) ) {
				throw new Exception( __( 'Ваша учетная запись пользователя не имеет права на регенерацию изображений.', 'force-regenerate-thumbnails' ) );
			}
			...
			// Не могу получить путь к изображению
			if ( false === $image_fullpath || strlen( $image_fullpath ) == 0 ) {

				// Попробуйте получить путь к изображению из url
				if ( ( strrpos( $image->guid, $upload_dir['baseurl'] ) !== false ) ) {
					...
					if ( realpath( $image_fullpath ) === false ) {
						throw new Exception( sprintf( __( 'Первоначально загруженный файл изображения не может быть найден по адресу &quot;%s&quot;.', 'force-regenerate-thumbnails' ), esc_html( (string) $image_fullpath ) ) );
					}
				} else {
					throw new Exception( __( 'Первоначально загруженный файл изображения не может быть найден.', 'force-regenerate-thumbnails' ) );
				}

			}

			// Image path incomplete
			// Путь к изображению неполон.
			...
			// Results
			$thumb_deleted = $thumb_error = $thumb_regenerate = [];
			...
			$args = [
				'crop'       => true,
				'sizes'      => $this->wpp_sizes,
				'water_mark' => true,
				'retina'     => true
			];

			$image_url = wp_get_attachment_url( $image->ID );
			$ff        = n_wpp_fr_image_bundle( $image_url, $args );

			if ( ! empty( $ff ) ) {
				foreach ( $ff as $file_size => $one_file ) {
					if ( ! empty( $one_file['thumb']['norm'] ) ) {
						$thumb_regenerate[] = $file_size;
					}
				}
			}
