...
class MB_User_Profile_Form {
	...
	public function render() {
		...
		echo '<form class="rwmb-form login100-form validate-form" method="post" enctype="multipart/form-data" encoding="multipart/form-data">';
	 ...
	 *
	 * @return array Array( error, object_id )
	 */
	public function process() {
		...
		if ( ! $is_valid ) {
			return array(
				'error'   => __( 'Invalid form submit.', 'mb-user-profile' ),
				'user_id' => 0,
			);
		}
	...
	protected function display_errors() {
		if ( isset( $_COOKIE['mb_user_profile_error'] ) ) {
			echo '<div class="rwmb-profile-error alert alert-danger">', wp_kses_post( $_COOKIE['mb_user_profile_error'] ), '</div>';
		}
	}
	...
	protected function register_scripts() {
		...
		wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
			...
			'current'       => __( 'Current Color', 'mb-user-profile' ),
		) );
	}
